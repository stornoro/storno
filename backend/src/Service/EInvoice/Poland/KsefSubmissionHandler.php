...
#[AutoconfigureTag('app.einvoice_submission_handler', ['provider' => 'ksef'])]
final class KsefSubmissionHandler implements EInvoiceSubmissionHandlerInterface
{
    public function __construct(
        private readonly KsefXmlGenerator $xmlGenerator,
        private readonly KsefValidator $validator,
    ...
    public function handle(Invoice $invoice, EInvoiceSubmission $submission): void
    {
        ...
        $configData = $company !== null
            ? $this->configResolver->resolve($company, EInvoiceProvider::KSEF)
            : [];

        if (!empty($configData['authToken']) && !empty($configData['nip'])) {
            ...
            $submission->setErrorMessage('KSeF: ' . $response->errorMessage);
            $submission->setMetadata(['xmlPath' => $xmlPath]);
            $this->entityManager->flush();
            return;
        }

        // No KSeF credentials — XML generation only
        $submission->setStatus(EInvoiceSubmissionStatus::SUBMITTED);
        $submission->setMetadata([
            'xmlPath' => $xmlPath,
            'xmlGeneratedAt' => (new \DateTimeImmutable())->format('c'),
            'note' => 'KSeF FA(2) XML generated. No KSeF credentials configured — upload manually.',
        ]);
        $this->entityManager->flush();
    }
    ...
    private function storeXml(Invoice $invoice, string $xml): string
    {
        ...
        $xmlPath = $cif . '/einvoice/ksef/' . $year . '/' . $month . '/' . $invoice->getId() . '.xml';

        $storage = $this->storageResolver->resolveForCompany($company);
        $storage->write($xmlPath, $xml);

        $this->logger->info('KsefSubmissionHandler: XML stored.', [
