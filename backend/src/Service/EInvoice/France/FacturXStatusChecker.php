...
#[AutoconfigureTag('app.einvoice_status_checker', ['provider' => 'facturx'])]
final class FacturXStatusChecker implements EInvoiceStatusCheckerInterface
{
    ...
    public function check(EInvoiceSubmission $submission, CheckEInvoiceStatusMessage $message): void
    {
        ...
        try {
            ...
            $this->logger->error('FacturXStatusChecker: Status check failed.', [
                'submissionId' => $message->submissionId,
                'error' => $e->getMessage(),
            ]);
