  ...
  <UModal v-model:open="isOpen" :ui="{ content: 'sm:max-w-3xl' }">
    <template #header>
      <div class="flex items-center gap-2">
        ...
        <h3 class="font-semibold">{{ $t('deliveryNotes.sendEmail') }}</h3>
      </div>
    </template>
    <template #body>
      <div class="space-y-5">
        <!-- Delivery note context -->
        <div class="rounded-lg bg-(--ui-bg-elevated) px-4 py-3 flex items-center justify-between">
          <div class="flex items-center gap-3 min-w-0">
            <UIcon name="i-lucide-package" class="size-4 shrink-0 text-(--ui-text-muted)" />
            <div class="min-w-0">
              <span class="font-medium">{{ deliveryNote.number }}</span>
              <span class="text-(--ui-text-muted) mx-1.5">&middot;</span>
              <span class="text-sm text-(--ui-text-muted)">{{ deliveryNote.clientName }}</span>
            </div>
          </div>
          <span class="font-semibold shrink-0">{{ formatMoney(deliveryNote.total, deliveryNote.currency) }}</span>
        </div>
...
watch(isOpen, async (open) => {
  if (open) {
    ...
    await emailTemplateStore.fetchTemplates('delivery_note')
    const defaults = await deliveryNoteStore.fetchEmailDefaults(props.deliveryNote.id)
...
async function onSend() {
  ...
  sending.value = true
  const result = await deliveryNoteStore.sendEmail(props.deliveryNote.id, {
    to: form.value.to,
  ...
  if (result) {
    ...
    toast.add({
      title: $t('deliveryNotes.emailSent'),
    ...
    toast.add({
      title: $t('deliveryNotes.emailError'),
      description: deliveryNoteStore.error || undefined,
