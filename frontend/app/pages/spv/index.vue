...
const requestTypeItems = computed(() => {
  ...
  for (const g of ['rapoarte', 'documente', 'declaratii', 'decizii']) {
    const items = store.requestTypes.filter(t => t.group === g).map(t => ({ label: t.label, value: t.type, group: g }))
      ...
      <UModal v-model:open="requestOpen" :title="$t('spv.requests.new')">
        <template #body>
          <div class="space-y-4">
            ...
            <p v-if="selectedRequestType && !selectedRequestType.wsSupported" class="text-xs text-muted flex items-center gap-1"><UIcon name="i-lucide-globe" class="size-3.5" />{{ $t('spv.requests.webOnlyHint') }}</p>
